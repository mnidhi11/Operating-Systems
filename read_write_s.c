...
int rwc = 0, wwc = 0, rc = 0, wc = 0, global_ID=1;

void reader_entry(int ID)
{
	printf("[reader: #%d]\ttrying to read\n", ID);
	P(mutex);
		//printf("Entering mutex of read\n");
	...
	rc++;
	//	printf("incrementing rc mutex of read%d\n",rc);
	if (rwc > 0){
		printf("read wait count is%d\n",rwc);
	    V(r_sem);
	 }
	else
	    V(mutex);		// let other processes use the mutex.
...
{
	P(mutex);  // i'm no longer a reader
	rc--;			printf("i'm no longer a reader%d\n",rc);
	if (rc == 0 && wwc > 0) {
